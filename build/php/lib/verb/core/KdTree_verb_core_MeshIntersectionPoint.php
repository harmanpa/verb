...
class KdTree_verb_core_MeshIntersectionPoint {
	/**
	 * @var int
	 */
	public $dim;
	 ...
	 * 
	 * @return KdNode_verb_core_MeshIntersectionPoint
	 */
	public function buildTree ($points, $depth, $parent) {
		...
		if ($points->length === 1) {
			...
			return new KdNode_verb_core_MeshIntersectionPoint(($points->arr[0] ?? null), $dim, $parent);
		}
		...
		$node = new KdNode_verb_core_MeshIntersectionPoint(($points->arr[$median] ?? null), $dim, $parent);
	...
	public function nearest ($point, $maxNodes, $maxDistance) {
		...
		$bestNodes = new BinaryHeap_verb_core_KdNode_verb_core_MeshIntersectionPoint(function ($e) {
		...
		$nearestSearch = function ($node) use (&$bestNodes, &$nearestSearch, &$point, &$_gthis, &$maxNodes) {
			...
			$saveNode = function ($node, $distance) use (&$bestNodes, &$maxNodes) {
				...
				$bestNodes->push(new HeapElement_verb_core_KdNode_verb_core_MeshIntersectionPoint($node, $distance));
		...
		while ($_g < $_g1) {
			...
			$bestNodes->push(new HeapElement_verb_core_KdNode_verb_core_MeshIntersectionPoint(null, $maxDistance));
		}
		...
		while ($_g < $_g1) {
			...
			if (($bestNodes->content->arr[$i] ?? null)->obj !== null) {
				...
				$x = new HeapElement_verb_core_KdPoint_verb_core_MeshIntersectionPoint(($bestNodes->content->arr[$i] ?? null)->obj->kdPoint, ($bestNodes->content->arr[$i] ?? null)->value);
...
Boot::registerClass(KdTree_verb_core_MeshIntersectionPoint::class, 'verb.core.KdTree_verb_core_MeshIntersectionPoint');
